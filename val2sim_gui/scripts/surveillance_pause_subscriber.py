...
# def surveillance_pause():
# 	pause_node = roslaunch.core.Node(package='val2sim_fsm', 
# 									node_type='val2sim_fsm_surveillance_pause.py', 
# 									name='val2sim_fsm_surveillance_pause_node',
# 									output='screen')
...
class surveillance_pause_publisher():
...
if __name__=="__main__":
	rospy.init_node('surveillance_pause_subscriber_node')
	guiSub_node = gui_pause_subscriber()
	surveillancePause_node = surveillance_pause_publisher()
	...
	try:
		while(not rospy.is_shutdown()):
			...
			if command == "pause":
				surveillancePause_node.publish_velocity()
			r.sleep()
